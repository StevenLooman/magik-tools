...
public class FormattingWalker extends AstWalker {
    ...
    protected void walkPreDefault(final AstNode node) {
        this.getStrategies()
            .forEach(strategy -> strategy.walkPreNode(node));
    }

    @Override
    protected void walkPostDefault(final AstNode node) {
        this.getStrategies()
            .forEach(strategy -> strategy.walkPostNode(node));
    }
    // endregion
    ...
    private void walkCommentToken(final Token token) {
        // Fixer upper: If comment token contains trailing whitespace, split the token and process separately.
        ...
        if (comment.length() != trimmedComment.length()) {
            final Token commentToken = Token.builder(token)
                .setValueAndOriginalValue(trimmedComment)
                .build();
