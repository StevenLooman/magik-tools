 ...
 *
 * <p>
 * Each MUnitTestItem has a unique ID which is unique within its parent. There is no guarantee that test items are
...
public class MUnitTestItem {
    ...
    /**
     * Add a child. If a child with the same id already exists, that child is returned and the new child is not added.
    ...
    public MUnitTestItem addChild(final MUnitTestItem newChild) {
        Optional<MUnitTestItem> optionalChild = this.children.stream()
