...
/** product.def {@link Check} class. */
public abstract class ProductDefCheck implements Check {
  ...
  protected ProductDefFile getProductDefFile() {
  ...
  public List<Issue> scanFileForIssues(final OpenedFile openedFile) {
    if (!(openedFile instanceof ProductDefFile productDefFile)) {
      throw new IllegalArgumentException("Expected ProductDefFile, got: " + openedFile.getClass());
    }
