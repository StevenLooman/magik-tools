...
/** load_list.txt/patch_list.txt {@link Check} class. */
public abstract class LoadListCheck implements Check {

  private static final Logger LOGGER = LoggerFactory.getLogger(LoadListCheck.class);
  ...
  protected LoadListFile getLoadListFile() {
  ...
  public List<Issue> scanFileForIssues(final OpenedFile openedFile) {
    if (!(openedFile instanceof LoadListFile loadListFile)) {
      throw new IllegalArgumentException("Expected LoadListFile, got: " + openedFile.getClass());
    }
