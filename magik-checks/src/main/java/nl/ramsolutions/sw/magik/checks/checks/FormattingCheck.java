...
public class FormattingCheck extends MagikCheck {
  ...
  /**
   * Require a non-whitespace-character before. If only whites-space is encountered before, this is
   * accepted as well.
   *
  ...
  private void requireMaxNewlines(final Token token) {
    ...
    if (fromToken != null && toToken != null) {
      ...
      this.addIssue(fromToken.getLine(), 0, toToken.getLine() + 1, toToken.getColumn(), message);
    }
  }
