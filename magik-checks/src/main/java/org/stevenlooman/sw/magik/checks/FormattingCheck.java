...
public class FormattingCheck extends MagikCheck {
  ...
  private void requireNonWhitespaceBefore(Token token) {
    int lastTokenLine = lastToken.getLine();
    int lastTokenEndColumn = lastToken.getColumn() + lastToken.getOriginalValue().length();
    if (lastTokenLine == token.getLine()
        && lastTokenEndColumn < token.getColumn()) {
  ...
  private void requireWhitespaceBefore(Token token) {
    ...
    int lastTokenEndColumn = lastToken.getColumn() + lastToken.getOriginalValue().length();
    if (lastTokenLine == token.getLine()
        && lastTokenEndColumn != token.getColumn() - 1) {
