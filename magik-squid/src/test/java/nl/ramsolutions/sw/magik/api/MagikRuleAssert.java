...
public abstract class MagikRuleAssert extends RuleAssert {
    ...
    protected ParseRunner createParseRunnerWithEofMatcher() {
        ...
        final MutableParsingRule endOfInput =
            (MutableParsingRule) new MutableParsingRule(new EndOfInput()).is(EndOfInputExpression.INSTANCE);
        final MutableParsingRule withEndOfInput =
            (MutableParsingRule) new MutableParsingRule(new WithEndOfInput(rule.getRuleKey())).is(actual, endOfInput);
     ...
     */
    @CheckForNull
    protected ParseNode findParseTreeWithRule(final ParseNode parseNode, final MagikGrammar findRuleKey) {
