...
/**
 * Test cases for {@link BlockIndentWalker}.
 *
 * <p>Tests verify that the formatter actively adds correct indentation to unindented code. Tab
 * width is set to 8 spaces (default) to match the Magik default.
 *
 * <p>Test naming convention:
 *
 * <ul>
 *   <li>testIndentXxx - tests that verify indentation is added to unindented code
 *   <li>testPreserveXxx - tests that verify correctly formatted code is not changed
 *   <li>testFixXxx - tests that verify incorrectly formatted code is corrected
 * </ul>
 */
class BlockIndentWalkerTest {
  ...
  private List<TextEdit> getEdits(final String code, final FormattingOptions options) {
    ...
    final FormattingProvider formattingProvider =
        new FormattingProvider(options, BlockIndentWalker.class);
  ...
  // ==========================================================================
  // Binary expressions (_andif, _orif)
  // ==========================================================================
  ...
  @Test
  void testIndentMixedAndifOrifWithPartialIndentation() {
    // Similar to testIndentMixedAndifOrif but some lines already have correct indentation.
    // The formatter should only add indentation to lines that don't have it.
    final String code =
        """
        _if a() _andif
        b() _orif
    ...
    // Lines 3 and 5 already have 4 spaces, so no edits for those lines.
  ...
  @Test
  void testIndentComplexMixedAndifOrif() {
    // Test with the user's actual code pattern - a complex condition with mixed _andif and _orif
    final String code =
        """
        _if obj.responds_to?(:property) _andif
        obj.property _is _false _orif
            obj.responds_to?(:property?) _andif
        obj.property? _is _false _orif
            obj.responds_to?(:property?) _andif
        obj.property? _is _false
        _then
        show(:not_property)
        _endif
        """;
    ...
    // All continuation lines should be indented to align with the first operand
    // Lines 3 and 5 already have 4 spaces, so those don't need edits
    // Lines 2, 4, 6 need 4 spaces added
  ...
  void testFixOverIndentedBlock() {
    final String code =
        ...
        """;
    final List<TextEdit> edits = this.getEdits(code);
    // Remove extra tab - edit starts at column 1, not 0
    assertThat(edits)
  ...
  void testFixMixedWhitespace() {
    // Space + tab should be normalized to just tab
    final String code =
        """
        _block
         \tprint(a)
  ...
  void testFixUnindentedMethodArguments() {
    // Test that method arguments at column 0 get properly indented even when
    // there's an assignment operator before the method call on the same line.
    final String code =
        """
        a << obj.method(
        :symbol_arg,
  ...
  @Test
  void testFixArgumentsOnSameLine() {
    final String code =
        """
        a.method1(x,
        \t  y,
    ...
    assertThat(edits)
        .containsExactly(
            new TextEdit(new Range(new Position(2, 1), new Position(2, 3)), ""), // fix \t  → \t
            new TextEdit(new Range(new Position(3, 1), new Position(3, 3)), "")); // fix \t  → \t
  }

  @Test
  void testPreserveArgumentsBlockIndentedOnSameLine() {
    final String code =
        """
        a.method1(x,
        \ty,
  ...
  @Test
  void testFixParametersOnSameLine() {
    final String code =
        """
        _method a.b(p_first,
        \t    p_second,
    ...
    assertThat(edits)
        .containsExactly(
            new TextEdit(new Range(new Position(2, 1), new Position(2, 5)), ""), // fix \t    → \t
            new TextEdit(new Range(new Position(3, 1), new Position(3, 5)), "")); // fix \t    → \t
  }

  @Test
  void testPreserveParametersBlockIndentedOnSameLine() {
    final String code =
        """
        _method a.b(p_first,
        \tp_second,
  ...
  // ==========================================================================
  // Method invocations (various patterns)
  // ==========================================================================
  ...
  // ==========================================================================
  // Invocation with result capture
  // ==========================================================================
