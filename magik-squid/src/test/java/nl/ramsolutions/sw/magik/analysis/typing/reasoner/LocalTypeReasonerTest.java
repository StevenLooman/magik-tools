...
class LocalTypeReasonerTest {
  ...
  void testGatherParameters() {
    ...
    final TypeString simpleVectorRef =
        TypeString.ofIdentifier(
            "simple_vector", "sw", TypeString.ofGenericDefinition("E", TypeString.UNDEFINED));
  ...
  void testGenericSlot() {
    final String code = "" + "_method exemplar.m\n" + "  _return .stack.pop()\n" + "_endmethod\n";
    ...
    definitionKeeper.add(
        new ExemplarDefinition(
            ...
            ExemplarDefinition.Sort.SLOTTED,
            exemplarRef,
            List.of(new SlotDefinition(null, null, null, null, "stack", slotTypeRef)),
            Collections.emptyList()));
  ...
  void testGenericSimpleVector() {
    ...
    final ExpressionResult result = reasonerState.getNodeType(methodDefinitionNode);
    final AbstractType actualType = result.get(0, null);
    assertThat(actualType.getTypeString())
        .isEqualTo(
            TypeString.ofIdentifier(
                TypeString.SW_SIMPLE_VECTOR.getIdentifier(),
                TypeString.SW_SIMPLE_VECTOR.getPakkage(),
