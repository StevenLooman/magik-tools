...
public class PatternExpression extends NativeExpression implements org.sonar.sslr.internal.matchers.Matcher {
  ...
  private static final Logger LOGGER = LoggerFactory.getLogger(PatternExpression.class);
  ...
  public void execute(Machine machine) {
    ...
    if (result) {
      final int start = machine.getIndex() + matcher.start();
      final int end = machine.getIndex() + matcher.end();
      String val = "";
      for (int offset = 0; offset < end - start; ++offset) {
        val += machine.charAt(offset);
      }

      System.err.println("Matched: " + this.toString().replace("\n", "\\n").replace("\r", "\\r") + ", val: " + val.replace("\n", "\\n").replace("\r", "\\r"));

      machine.createLeafNode(this, matcher.end());
      machine.jump(1);
    } else {
      System.err.println("Backtrack: " + this.toString().replace("\n", "\\n").replace("\r", "\\r"));
