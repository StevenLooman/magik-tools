 ...
 *
 * <p>Any line number is 0-based, as is in {@link MagikFile}.
 */
public class CommentInstructionReader {
  ...
  /**
   * Get instruction for node. Simply takes line number from node and gets instruction(s) at line.
   *
   * @param node Node.
        ...
        .filter(
            line ->
                ...
                scope.getChildScopes().stream()
                    .noneMatch(
                        childScope ->
                            line >= childScope.getStartLine() && line < childScope.getEndLine()))
  ...
  /**
   * Parse a instruction(s) with the form: `a=b; c=d`. An instruction is a key/value pair bound by a
   * `=` character. Instructions are separated with a `;`.
   *
