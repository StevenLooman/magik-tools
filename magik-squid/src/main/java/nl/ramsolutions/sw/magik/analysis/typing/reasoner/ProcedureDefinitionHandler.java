...
class ProcedureDefinitionHandler extends LocalTypeReasonerHandler {
  ...
  void handleProcedureDefinition(final AstNode node) {
    final AbstractType abstractProcedureType = this.typeKeeper.getType(TypeString.SW_PROCEDURE);
    ...
    for (final AstNode parameterNode : parameterNodes) {
      ...
      final AbstractType type =
          this.state.getNodeType(parameterNode).get(0, UndefinedType.INSTANCE);
      final TypeString typeString = type.getTypeString();
      final Parameter parameter =
          new Parameter(parameterLocation, identifier, modifier, typeString);
    ...
    final ExpressionResultString loopbodyResultStr =
        TypeReader.unparseExpressionResult(loopbodyResult);

    // Create procedure instance.
    final EnumSet<ProcedureInstance.Modifier> modifiers =
        EnumSet.noneOf(ProcedureInstance.Modifier.class);
    ...
    final ProcedureInstance procType =
