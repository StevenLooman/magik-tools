...
public final class AstCompare {
  ...
  /**
   * Flags to influence the compare functionality. Values: {@code IDENTIFIER_IGNORE_NAME}: Ignore
   * the name of the IDENTIFIER node.
   */
  enum Flags {
  ...
  /**
   * Test if two nodes are equal to each other. Compares token values and structure.
   *
  ...
  public static boolean astNodeEqualsRecursive(
      final AstNode left, final AstNode right, final Set<Flags> flags) {
    // Compare nodes.
  ...
  /**
   * Test if two nodes are equal to each other. Compares token values.
   *
   * @param left Node to compare
   * @param right Node to compare
   * @return True if nodes are equal, false otherwise.
  ...
  public static boolean astNodeEquals(
    ...
    if (flags.contains(Flags.IGNORE_IDENTIFIER_NAME) && AstCompare.isIdentifierNode(left)) {
