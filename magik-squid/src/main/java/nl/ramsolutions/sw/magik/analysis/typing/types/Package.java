...
public class Package {
  ...
  public Map<String, AbstractType> getTypes() {
    return this.typeKeeper.getTypes().stream()
        ...
        .collect(Collectors.toMap(AbstractType::getName, type -> type));
  }
  ...
  public String toString() {
    final String usesParts =
        this.getUsesPackages().stream().map(Package::getName).collect(Collectors.joining(","));
    return String.format(
        "%s@%s(%s, uses: %s)",
        this.getClass().getName(), Integer.toHexString(this.hashCode()), this.getName(), usesParts);
  }
}
