 ...
 * Position in a file.
 *
 * <p>Lines start at 1, columns start at 0.
 */
public class Position implements Comparable<Position> {
  ...
  public static Position fromTokenEnd(final Token token) {
    ...
    final Character lastChar =
        !tokenValue.isEmpty() ? tokenValue.charAt(tokenValue.length() - 1) : null;
    ...
    final int line = lines.length == 1 ? token.getLine() : token.getLine() + lines.length - 1;
    final int column =
        lines.length == 1 ? token.getColumn() + lastLine.length() : lastLine.length();
