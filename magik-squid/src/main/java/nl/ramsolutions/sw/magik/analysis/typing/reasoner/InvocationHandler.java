...
class InvocationHandler extends LocalTypeReasonerHandler {
    ...
    void handleMethodInvocation(final AstNode node) {
        ...
        if (methods.isEmpty()) {
            ...
            final List<TypeString> argumentTypeStrs = argumentExpressionNodes.stream()
            ...
            for (final Method method : methods) {
                ...
                final ExpressionResultString processedMethodCallResultStr = this.processExpressionResult(
                    originalCalledType,
                    calledType,
                    method,
                    methodCallResultStr,
                ...
                final ExpressionResultString processedMethodIterResultStr = this.processExpressionResult(
                    originalCalledType,
                    calledType,
                    method,
                    methodIterResultStr,
    ...
    private ExpressionResultString substituteParameterRefs(
        ...
        final Map<TypeString, TypeString> paramRefArgTypeRefMap = IntStream.range(0, parameters.size())
            .mapToObj(i -> {
                final Parameter param = method.getParameters().get(i);
                final String paramName = param.getName();
                final TypeString paramRef = TypeString.ofParameterRef(paramName);
                final TypeString argTypeRef = i < argumentTypes.size()
                    ? argumentTypes.get(i)
                    : TypeString.SW_UNSET;
    ...
    private ExpressionResultString processExpressionResult(
        ...
        newExpressionResultString = originalCalledType != SelfType.INSTANCE
            ? newExpressionResultString.substituteType(TypeString.SELF, calledTypeStr)
            : newExpressionResultString;

        // Substitute parameters.
        newExpressionResultString = this.substituteParameterRefs(method, argumentTypeStrs, newExpressionResultString);
