...
public class MethodArgumentCountTypedCheck extends MagikTypedCheck {
    ...
    protected void walkPostMethodInvocation(final AstNode node) {
        ...
        final boolean anyScatter = argumentsNode.getChildren(MagikGrammar.ARGUMENT).stream()
            .anyMatch(argumentNode -> {
                AstNode unaryExprNode = AstQuery.getFirstChildFromChain(node,
                    MagikGrammar.EXPRESSION,
                    MagikGrammar.UNARY_EXPRESSION);
                String tokenValue = unaryExprNode != null
                    ? unaryExprNode.getTokenValue()
                    : null;
