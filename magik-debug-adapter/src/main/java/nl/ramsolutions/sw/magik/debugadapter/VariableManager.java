...
class VariableManager {
    ...
    static class MagikVariable {
         ...
         */
        MagikVariable(final int id, final int frameId, final String name, final String value, final String expression) {
            this.id = id;
     ...
     * @return Variables.
     */
    List<MagikVariable> getVariables(final int reference) throws IOException, InterruptedException, ExecutionException {
        ...
        if (scope != null) {
            ...
            final String typeExpression = ""
                    + "_if " + expression + ".is_kind_of?(sw:enumerated_format_mixin) "
                    + "_then >> :enumerated_format_mixin "
                    + "_elif " + expression + ".is_kind_of?(sw:indexed_format_mixin) "
                    + "_then >> :indexed_format_mixin "
                    + "_elif " + expression + ".is_kind_of?(sw:slotted_format_mixin) "
            ...
            switch (typeEvalResponse.getResult()) {
                ...
                default:
                    LOGGER.warn(
                        ...
                        expression, typeEvalResponse.getResult());
    ...
    private List<MagikVariable> variablesFromSlotted(final MagikVariable variable)
        ...
        for (final String slotName : slotNames.split(",")) {
            ...
            final EvalResponse slotValueResponse =
                (EvalResponse) this.slapProtocol.evaluate(
                    threadId,
                    level,
                    slotValueExpression + ".print_string").get();
            final String slotValue = slotValueResponse.getResult();

            final MagikVariable slotVariable = this.addVariable(variable, slotName, slotValue, slotValueExpression);
        ...
        for (int i = 0; i < size; ++i) {
            ...
            final EvalResponse itemValueResponse =
                (EvalResponse) this.slapProtocol.evaluate(
                    threadId,
                    level,
                    itemValueExpression + ".print_string").get();
